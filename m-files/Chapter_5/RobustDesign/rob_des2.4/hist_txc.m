function hist_txt(vv,bins,vtxt,ttxt,opt,col);
% HIST_TXT: Plot a relative frequency histogram with title, xlabel & ylabel
% ============================================================================
% hist_txt(vv,[bins],[vtxt],[ttxt],[opt],[col]);
% ----------------------------------------------------------------------------
% vv    : Variable (vector)
...
if nargin<6   % check col, set to default
  col='b';
elseif length(col)==0,
  col='b';
end
...
ns=n./length(vv);
B=bar(xb,ns);   % calculate bars  original: bar('v6',xb,ns) changed Horst Rumpf 
xxx=get(B,'XData');
yyy=get(B,'YData');
