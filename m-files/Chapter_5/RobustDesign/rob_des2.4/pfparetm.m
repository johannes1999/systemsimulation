function [el,cog,ind] = pfparetm(yy,ytxt,lsl,usl,ttxt,sort_opt,opt,x,tx,index)

% PFPARETM: Calculate and plot a Pareto diagram (matrix version)
% ============================================================================
% [el,cog,ind] = pfparetm(yy,[ytxt],[lsl],[usl],[ttxt],[sort_opt],[opt],...
%                x,[tx],[index]);
% ----------------------------------------------------------------------------
...
% ----------------------------------------------------------------------------
% Example: pfparetm(yy,'yname',[],[],'Pareto stat. sensitivities',...
%                   1,[1,1,1,1],x,str2mat('l','b','h','x','E'),[1:3 5]);
% ----------------------------------------------------------------------------
...
for i=1:npar,
  [elvect(i) cogvect(i)]=passfail(x(index(i),:),yy,lsl,usl);
end
...
if opt(2)==0 | (opt(2)==1 & length(lsl)==0 & length(usl)==0),
  ...
  for i=1:nplot,
    text(2.1,i,sprintf('%-6.2G',cogvect(index_sort(start_plot+i-1))));
  end
