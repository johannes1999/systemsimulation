...
Example=6;  % 1: Häufigkeitsfunktion und Summenhäufigkeitsfunktion
...
%-------------------------
   ...
   n_s   =10000;            % Number of simulations
   AnzKl = n_bins(n_s);     % Number of bins
   Ts=1;                    % Abtastrate [s oder Wert]
     
   
   fall =3;                 % Different distortions
   
   A1_nom=5; A1_tol=0.1*A1_nom; A1=rnd_vect(n_s,'normal',[A1_nom A1_tol/3]);
   if fall==1
        A1=A1+0.05*sin(20*pi/n_s*(1:n_s));                  % add sine function
    elseif fall==2     
        A2=rnd_vect(n_s/10,'uniform',[A1_nom A1_tol/3]);      % add higher spread
        N1start=n_s/2-length(A2)/2; N1end=N1start+length(A2)-1;
        A1(N1start:N1end)=A2;       clear A2;
    elseif fall==3
        A2=rnd_vect(n_s/10,'normal',[A1_nom A1_tol]);     % add higher spread  
        N1start=n_s/2-length(A2)/2; N1end=N1start+length(A2)-1;
        A1(N1start:N1end)=A2;       clear A2;
    end 
   ...
   N=n_s/10;      % Number of time slots 
   P=0.8;           % Overlap max 0.8
   
   [t, x_bins, t_Histxy, t_Mean, t_Std]=t_hist(A1,1/Ts,N,P);
%   t_Histxy= scaling(t_Histxy,128); % scaling of data for best colour resolution
  
   
   % Time RMS
   %---------
   
    [t_rms, t_rmsxy, t_peak, t_rms_peak]=t_rms(A1,1/Ts,N,P);
   
   
   % Time-FFT
   %---------
   
    fmin=8/n_s;                                     % Kleinste Frequenz  die bei der FFT berechnet werden soll
    P_over=0.5;                                     % Überlappung bei P_over=1 ist die Überlappung =0
    Ts=1;                                           % Abtastrate [s oder Wert]
     
    [t1,f,Ux]=tfft(A1,1/Ts,P_over,fmin);            % Zeitspektrum 
    
   % Ux= scaling(20*log(Ux+0.0000001),128);          % scaling of data for best colour resolution
   ...
   % [f1, Ux1]=nfft(A1',1/Ts); 
      ...
      Uauto=(A1,'coeff');%,'coeff');
      
      
    figure
    subplot(3,2,1)
        plot(A1); grid
        %xlabel('Data Number  [-]');  ylabel('Value');  
        set(gca,'xscale','lin','xlim',[0 n_s]);
        title('Values')
    subplot(3,2,2)
        hist(A1,n_bins(n_s)); grid;   
    subplot(3,2,3)
        surf(x_bins,t,(t_Histxy));
        view([90 90]);
        shading interp; colormap(jet); 
        set(gca,'xscale','lin','xlim',[min(x_bins) max(x_bins)],'yscale','lin','ylim',[min(t) max(t)]);
        xlabel('Value');  % ylabel('Data Number  [-]');    %zlabel (' rel. Value (20*log(U)')
        title('Time Histogram')
   subplot(3,2,4)      
         plot(t,t_Mean,t,t_Mean+t_Std,'r-.',t,t_Mean-t_Std,'r-.'); grid
         set(gca,'xscale','lin','xlim',[0 n_s]);
         legend('mean-value','+/-S')
   subplot(3,2,5) 
        loglog(f1,Ux1); grid;
        set(gca,'xlim',[min(f1) max(f1)],'ylim',[1e-4 1.1*max(Ux1)]);
        title('Spectrum')
        
    subplot(3,2,6)
        surf(f,t1,Ux);
        view([90 90]);
        set(gca,'XDir','reverse'); %Dreht die Richtung der X-Achse im Diagramm um!
        shading interp; colormap(jet); 
        set(gca,'xscale','log','yscale','lin','ylim',[min(t1) max(t1)]);
         xlabel('f  [Hz]');   ylabel('N [-]');   % zlabel (' rel. Value (20*log(U)')
        title('Time FFT')
  
    figure
    subplot(2,2,1)
        plot( t_rms, t_rmsxy,'r'); grid
          ylabel('RMS');  title('Time RMS')
          set(gca,'xlim',[min(t_rms) max(t_rms)],'ylim',[min(t_rmsxy) max(t_rmsxy)]);
    subplot(2,2,2)
        plot( t_rms, t_peak,'b'); grid 
         ylabel('Peak'); title('Peak value')
         set(gca,'xlim',[min(t_rms) max(t_rms)],'ylim',[min(t_peak) max(t_peak)]);
    subplot(2,2,3)
        plot( t_rms, t_rms_peak,'g'); grid 
        ylabel('RMS/Peak');   xlabel('Number of Data  [-]'); title('RMS/Peak')
        set(gca,'xlim',[min(t_rms) max(t_rms)],'ylim',[min(t_rms_peak) max(t_rms_peak)]);
   subplot(2,2,4)
        plot( Uauto); grid 
        ylabel('RMS/Peak');   xlabel('Number of Data  [-]'); title('RMS/Peak')
end
